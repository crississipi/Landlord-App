...
const DropDownBtn = ({ list, onSelect, defaultValue }: DropDownProps) => {
  ...
  return (
    <div ref={dropdownRef} className='h-auto w-auto min-w-32 md:min-w-40 flex flex-col relative rounded-sm md:rounded-md shadow-sm'>
        <button 
        ...
        >
            <span className='text-sm truncate'>{currCat}</span>
            <AiOutlineCaretDown 
        ...
        {isOpen && (
          ...
          >
            {list.map((item, index) => (
                <button 
                  key={index} 
                  className={`click__action outline-none py-2 md:py-2.5 text-sm w-full px-3 md:px-4 text-left transition-colors duration-150 ${
                    item === currCat 
