...
const TitleButton: React.FC<SetBtn> = ({ title, setPage }) => {
  ...
  return (
    <button className='flex__center__y click__action mt-1 h-9 w-auto outline-none rounded-sm text-emerald-700 gap-2 group' onClick={() => changePage(0)}>
        <div className='click__action rounded-sm min-h-full w-9 p-1 group-focus:ring-2 group-focus:ring-customViolet/50 group-focus:scale-105 group-hover:ring-2 group-hover:ring-customViolet/20'>
