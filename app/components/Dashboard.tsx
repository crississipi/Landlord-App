...
const Dashboard = ({ inDbPage }: DashboardProps ) => {
  ...
  return (
    <div className='h-auto w-full flex flex-col gap-6 md:grid md:grid-cols-12 text-customViolet p-4 md:p-6'>
      ...
      <div className={`${!inDbPage ? 'grid-rows-11 md:col-span-7 md:h-auto lg:col-span-4 lg:gap-4' : 'grid-rows-12 md:col-span-7 md:h-auto lg:col-span-5'} primary__btn__holder text-customViolet text-sm`}>
        <DashboardCard 
          size={`col-span-6  ${inDbPage ? 'row-span-full' : 'row-span-8 md:row-span-7 lg:row-span-8'}`} 
          title="Net Revenue" 
          rate={true} 
          value="9,735" 
          priceStyle="text-3xl font-bold w-full h-full flex items-center md:text-4xl lg:text-5xl text-customViolet"
