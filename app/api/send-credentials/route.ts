...
const sendTenantCredentials = async (credentials: EmailCredentials): Promise<{ success: boolean; error?: string }> => {
  try {
...
export async function POST(request: Request) {
  try {
    ...
    const { to, firstName, lastName, unitNumber, username, password } = body;

    // Validate required fields
    if (!to || !firstName || !lastName || !unitNumber || !username || !password) {
